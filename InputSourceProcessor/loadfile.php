...
$text = "";
$time = 0;
$addsource = $mysql->prepare("INSERT IGNORE INTO cs125.sources (`type`,`text`,`time`,`score`) VALUES (?,?,?,NULL)");
$addsource->bind_param("isi", $type, $text, $time);


if( count($argv) != 2 || !file_exists($argv[1]) ) die("Pass me the CSV file!");
...
foreach($lines as $msg) {
	if( substr_count($msg, "|") == 2 ) {
		...
		if( $time && $time > 1457317042 && strlen($msg) > 15 && strlen($msg) < 100 ) {
